...
class ThongKeCauTraLoi extends Controller
{
    ...
    {
        // Lấy tất cả các kết quả từ bảng KetQua
        $ketQuas = KetQua::all();

        // Tách chuỗi thành mảng dựa trên dấu "/"
        $mang = explode("/", $ketQuas->Diem);

        // Lấy phần tử đầu tiên là số câu đúng
        $soCauDung = $mang[0];

        // Lấy phần tử thứ hai là tổng số câu hỏi
        $tongSoCau = $mang[1];
        // Tính tổng số lượng kết quả
        $tongSoLuong = $ketQuas->count();

        // Tính tổng điểm
        $tongDiem = $ketQuas->sum('diem');

        // Truyền dữ liệu tới view
        return view('thongke', compact('ketQuas', 'tongSoLuong', 'tongDiem'));
    }
}
